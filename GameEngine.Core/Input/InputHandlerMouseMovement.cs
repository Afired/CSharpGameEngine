...
public partial class InputHandler {
    ...
    public unsafe void ResetMouseDelta(WindowHandle* window) {
        ...
        if(_catchCursor)
            Glfw.SetCursorPos(window, (double) Application.Instance!.Config.WindowWidth / 2d, (double) Application.Instance!.Config.WindowHeight / 2d); //todo: not config but current glfw window size
    }
